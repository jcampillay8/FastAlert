...
* `fastalert` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `FastAlert` - executable or bundle name on disk.
* `FastAlert` - display name shown in window titles and about app dialogs.
* `com.vecinos.fastalert` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
